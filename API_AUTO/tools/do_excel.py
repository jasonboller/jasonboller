        ...
        for key in mode: # 遍历这个存在配置文件里面的字典
            ...
            if mode[key] == 'all':
                for i in range(2, sheet.max_row+1):
                    ...
                    elif sheet.cell(i, 4).value.find('${tel}') != -1:
                        sub_data['data'] = sheet.cell(i, 4).value.replace('${tel}', str(tel+1))
                    else: # 如果没有找到的话
                        sub_data['data'] = sheet.cell(i, 4).value
                ...
                for case_id in mode[key]:
                    ...
                    elif sheet.cell(case_id+1, 4).value.find('${tel}') != -1:
                        sub_data['data'] = sheet.cell(case_id+1, 4).value.replace('${tel}', str(tel+1))
                    else: # 如果没有找到的话
                        sub_data['data'] = sheet.cell(case_id+1, 4).value
